...
export const insert = (record) =>
    async (database) => {
        supabase.from(database.name).insert(record).select()
        return local_database.insert(record)(database)
    }

export const remove = ({column, value}) =>
    async (database) => {
        supabase.from(database.name).delete().eq(column, value).select()
...
export const update = ({column, value}, updater) =>
    async (database) => {
        ...
        supabase.from(database.name).update(updated_value).eq(column, value).select()
